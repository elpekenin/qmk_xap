...
pub fn draw(device: &XAPDevice, user_data: &mut UserData) {
    ...
    if notifications == 0 {
        gui::draw::rect(device, SCREEN_ID, X, Y, X + IMG_SIZE, Y + IMG_SIZE, gui::BG_COLOR, true);
        return;
    }

    gui::draw::image(device, SCREEN_ID, X, Y, IMG);
    gui::draw::text_recolor(device, SCREEN_ID, X + IMG_SIZE - FONT_SIZE , Y, FONT, gui::HSV_WHITE, gui::HSV_RED, format!("{}", notifications))
}
